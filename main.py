...
MIN_DEP_USD = float(os.getenv("MIN_DEP_USD", "50"))

# $50 ekvivalenti (taxminiy fallback). Backend MIN_DEP_USD = 50 asosida.
# Kurs o‘zgarsa faqat shu jadvalni yangilang — tekshiruv dollar bo‘yicha.
DEP_AMOUNT = {
    "uz": "635 000 so‘m ($50)",
    "ru": "4 600 ₽ ($50)",
    "en": "$50",
...
def dep_label(lang: str) -> str:
    return DEP_AMOUNT.get(lang) or DEP_AMOUNT.get("en", "$50")
...
T = {
    "uz": {
        ...
        "welcome": (
            ...
            "📈 Bashorat aniqligi: <b>100%</b> (va u doimiy ravishda yaxshilanmoqda).\n\n"
        ...
        "reg_ok": (
            ...
            "Signallarga kirish uchun kamida <b>{dep_label}</b> depozit qiling. 💳"
        ),
        "dep_need": (
            "🌐 <b>Signalga kirish uchun kamida {dep_label} depozit qiling.</b>\n\n"
        ...
        "open_app_text": (
            ...
            "Signal olish uchun 1WIN da ro‘yxat + kamida <b>{dep_label}</b> depozit kerak "
            "(mini-app ichidagi «Mening hisobim»)."
        ),
    ...
    "ru": {
        ...
        "welcome": (
            ...
            "📈 Точность прогнозов: <b>100%</b> (постоянно улучшается).\n\n"
        ...
        "reg_ok": (
            ...
            "Для доступа к сигналам внесите минимум <b>{dep_label}</b>. 💳"
        ),
        "dep_need": (
        ...
        "open_app_text": (
            ...
            "Для сигнала: регистрация 1WIN + депозит от <b>{dep_label}</b> "
            "(раздел «Мой аккаунт» в mini-app)."
        ),
    ...
    "en": {
        ...
        "welcome": (
            ...
            "📈 Prediction accuracy: <b>100%</b> (constantly improving).\n\n"
        ...
        "reg_ok": (
            ...
            "Deposit at least <b>$50</b> to unlock signals. 💳"
        ),
        "dep_need": (
        ...
        "open_app_text": (
            ...
            "To get signals: register on 1WIN + deposit at least <b>{dep_label}</b> "
            "(see «My Account» in the mini-app)."
        ),
    ...
    "tr": {
        ...
        "welcome": (
            ...
            "📈 Tahmin doğruluğu: <b>%100</b> (sürekli iyileşiyor).\n\n"
...
LEVELS_TEXT = (
    "📊 <b>Signal aniqligi:</b>\n"
    "————————————\n"
    "🎯 Barcha foydalanuvchilar uchun: <b>100%</b>\n"
    "💰 Mini-app uchun minimal depozit: <b>$50</b>"
)

def dep_need_text(lang: str) -> str:
    """Minimal $50 depozit talabi — tilga mos summa."""
    amt = dep_label(lang)
    texts = {
        "uz": (
            f"🌐 <b>Signallarga kirish uchun kamida {amt} depozit qilishingiz kerak.</b>\n\n"
            f"✦ Minimal depozit: <b>{amt}</b>\n"
            f"✦ Undan kam bo‘lsa — mini-app ochilmaydi.\n"
            f"✦ Mablag‘lar HISOBINGIZGA kiritiladi.\n\n"
            f"● Depozitdan so‘ng «🔍 Depozitni tekshirish» tugmasini bosing."
        ),
        "ru": (
            f"🌐 <b>Для доступа к сигналам нужен депозит минимум {amt}.</b>\n\n"
            f"✦ Минимальный депозит: <b>{amt}</b>\n"
            f"✦ Меньше — мини-приложение не откроется.\n"
            f"✦ Средства зачисляются на ВАШ счёт.\n\n"
            f"● После депозита нажмите «🔍 Проверить депозит»."
        ),
        "en": (
            f"🌐 <b>A minimum deposit of {amt} is required to access signals.</b>\n\n"
            f"✦ Minimum deposit: <b>{amt}</b>\n"
            f"✦ Less than that — mini-app will not open.\n"
            f"✦ Funds go to YOUR account.\n\n"
            f"● After deposit tap «🔍 Check deposit»."
        ),
        "tr": (
            f"🌐 <b>Sinyallere erişim için minimum {amt} yatırım gerekli.</b>\n\n"
            f"✦ Minimum yatırım: <b>{amt}</b>\n"
            f"✦ Daha az olursa mini uygulama açılmaz.\n\n"
            f"● Yatırımdan sonra «🔍 Yatırımı kontrol et»e basın."
        ),
    }
    return texts.get(lang) or texts["en"]

DEP_NEED_FULL = dep_need_text("uz")  # fallback (handlers tilga qarab chaqiradi)
...
async def on_account(cq: CallbackQuery, bot: Bot):
    try:
        ...
        dep_ok = bool(user.get("deposited"))
        text = (
            f"👤 <b>Profil</b>\n————————————\n"
            f"🆔 ID: <code>{cq.from_user.id}</code>\n"
            f"💰 Depozit: {'✅ ' + dep_label(lang) if dep_ok else '❌ (min ' + dep_label(lang) + ')'}\n"
            f"🎯 Signal aniqligi: <b>100%</b>\n"
            f"————————————\n"
...
async def on_signal(cq: CallbackQuery, bot: Bot):
    ...
    text = t(lang, "open_app_text", dep_label=dep_label(lang))
    if not text or text == "open_app_text":
        text = t(lang, "dep_ok")
...
async def on_dep_how(cq: CallbackQuery):
    user = await get_user(cq.from_user.id)
    lang = user.get("lang") or "uz"
    await safe_screen(cq, dep_need_text(lang), kb_deposit_check(lang, cq.from_user.id), PHOTO_DEP)
    await cq.answer()
        ...
        if event in ("reg", "registration", "register"):
            ...
            try:
                ...
                await bot.send_message(
                    tg_id,
                    t(lang, "reg_ok", dep_label=dep_label(lang)),
            ...
            # Faqat amount >= MIN_DEP_USD ($50) bo‘lsa aktivlashadi
            try:
                amt_val = float(str(amount).replace(",", ".").replace(" ", "") or "0")
            except Exception:
                amt_val = 0.0
            user = await get_user(tg_id)
            lang = user.get("lang") or "uz"
            if amt_val + 1e-9 >= MIN_DEP_USD:
                ...
                try:
                    ...
                    await bot.send_message(
                        tg_id,
                        t(lang, "dep_ok"),
                        reply_markup=kb_open_app(lang),
                        protect_content=True,
                    )
                except Exception as e:
                    logger.warning("notify dep failed: %s", e)
            else:
                # Kam depozit — faollashtirmaymiz, ogohlantiramiz
                try:
                    bot: Bot = request.app["bot"]
                    need = dep_label(lang)
                    await bot.send_message(
                        tg_id,
                        f"⚠️ Depozit qabul qilindi, lekin minimal miqdor {need}.\n"
                        f"Hozirgi: ${amt_val:g}. Yana to‘ldiring.",
                        reply_markup=kb_deposit_check(lang, tg_id),
                        protect_content=True,
                    )
                except Exception as e:
                    logger.warning("notify low dep failed: %s", e)
