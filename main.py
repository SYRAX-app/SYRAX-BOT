...
async def safe_screen(cq: CallbackQuery, text: str, reply_markup=None, photo=None, edit=True):
    """Rasm/matn ekran — avvalgi xabarni o'chiradi yoki edit qiladi"""
    chat_id = cq.message.chat.id
    bot = cq.bot
    
    # Agar photo bo'lsa - eski xabarni o'chirib yangi yuboramiz
    if photo:
        text = (text or "")[:1024]
        try:
    ...
    # Agar eski xabar photo bo'lsa - o'chirib yangi text yuboramiz
    if cq.message.photo:
        try:
            await cq.message.delete()
        except Exception:
    ...
    # Text xabarni edit qilamiz (agar edit=True bo'lsa)
    if edit:
        try:
    ...
    # Agar edit bo'lmasa yoki xatolik bo'lsa - yangi xabar yuboramiz
    try:
        await cq.message.delete()
    except Exception:
        pass
...
async def on_lang(cq: CallbackQuery, bot: Bot):
    ...
    if sub:
        ...
        await safe_screen(cq, welcome_text(lang, name), kb_main(lang), PHOTO_START, edit=False)
    else:
        await safe_screen(cq, t(lang, "need_sub"), kb_sub(lang), edit=False)
    await cq.answer()
...
async def on_set_lang(cq: CallbackQuery):
    await safe_screen(cq, t("uz", "choose_lang"), kb_langs(), edit=False)
    await cq.answer()
...
async def on_guide(cq: CallbackQuery):
    user = await get_user(cq.from_user.id)
    lang = user.get("lang") or "uz"
    await safe_screen(cq, t(lang, "guide"), kb_guide(lang), PHOTO_GUIDE, edit=False)
    await cq.answer()


@router.callback_query(F.data == "video")
async def on_video(cq: CallbackQuery):
    """Faqat video — ostida hech qanday yozuv yo‘q."""
    user = await get_user(cq.from_user.id)
    lang = user.get("lang") or "uz"
    try:
        await cq.message.delete()
        await cq.bot.send_video(
            cq.message.chat.id,
            video=VIDEO_GUIDE,
        ...
        await safe_screen(cq, t(lang, "video", support=SUPPORT_URL), kb_back(lang), edit=False)
    await cq.answer()
...
async def on_account(cq: CallbackQuery, bot: Bot):
    try:
        ...
        await safe_screen(cq, text, kb_account(lang), edit=False)
    except Exception as e:
        logger.exception("account: %s", e)
        await safe_screen(cq, f"👤 ID: <code>{cq.from_user.id}</code>", kb_back("uz"), edit=False)
    await cq.answer()
...
async def on_levels(cq: CallbackQuery):
    user = await get_user(cq.from_user.id)
    lang = user.get("lang") or "uz"
    await safe_screen(cq, LEVELS_TEXT, kb_levels(lang), edit=False)
    await cq.answer()
...
async def on_signal(cq: CallbackQuery, bot: Bot):
    ...
    if not user.get("deposited"):
        await safe_screen(cq, DEP_NEED_FULL, kb_deposit_check(lang, tg_id), PHOTO_DEP, edit=False)
        await cq.answer()
        return
    if not user.get("balance_ok", 1):
        await safe_screen(cq, t(lang, "balance_zero"), kb_deposit(lang, tg_id), PHOTO_DEP, edit=False)
        await cq.answer()
        return
    await safe_screen(cq, t(lang, "dep_ok"), kb_open_app(lang), edit=False)
    await cq.answer()
...
async def on_link_fail(cq: CallbackQuery):
    user = await get_user(cq.from_user.id)
    lang = user.get("lang") or "uz"
    await safe_screen(cq, t(lang, "link_fail_help", support=SUPPORT_URL), kb_reg(lang, cq.from_user.id), edit=False)
    await cq.answer()
...
async def on_check_id(cq: CallbackQuery):
    ...
    await safe_screen(cq, t(lang, "send_id"), kb_back(lang), edit=False)
    await cq.answer()
...
async def on_dep_how(cq: CallbackQuery):
    user = await get_user(cq.from_user.id)
    lang = user.get("lang") or "uz"
    await safe_screen(cq, DEP_NEED_FULL, kb_deposit_check(lang, cq.from_user.id), PHOTO_DEP, edit=False)
    await cq.answer()
...
async def on_text(message: Message):
    ...
    try:
        await message.answer_photo(PHOTO_START, caption=welcome_text(lang, name)[:1024], reply_markup=kb_main(lang))
    except Exception:
        await message.answer(welcome_text(lang, name), reply_markup=kb_main(lang))
