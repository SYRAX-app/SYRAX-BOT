...
async def safe_screen(cq: CallbackQuery, text: str, reply_markup=None, photo=None):
    """Eski menyuni o‘chirib, yangisini ochadi (stack bo‘lmasin)."""
    if photo:
        text = (text or "")[:1024]
    chat_id = cq.message.chat.id
    bot = cq.bot
    # Har doim eski xabarni o‘chiramiz — photo/video/text farqi yo‘q
    try:
    ...
    try:
        ...
        logger.warning("safe_screen: %s", e)
        try:
            await bot.send_message(chat_id, text or "OK", reply_markup=reply_markup)
        except Exception:
            pass
...
async def on_video(cq: CallbackQuery):
    """Video — ostida yozuv yo‘q, faqat Asosiy menyu tugmasi."""
    user = await get_user(cq.from_user.id)
    lang = user.get("lang") or "uz"
    try:
        try:
        ...
        try:
            await cq.bot.send_message(
                cq.message.chat.id,
                t(lang, "video", support=SUPPORT_URL),
                reply_markup=kb_back(lang),
            )
        except Exception:
            pass
