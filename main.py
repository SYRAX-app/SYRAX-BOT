...
DEP_NEED_FULL = (
    ...
    "✦ Depozit miqdori botdagi LVL (daraja), status va signal muvaffaqiyati "
    "ehtimoliga bog‘liq. Katta depozit qanchalik katta bo‘lsa, botdagi "
    "LVLingiz shunchalik yuqori bo‘ladi va muvaffaqiyat ehtimoli yuqori "
    "bo‘lgan ko‘proq signallarni qabul qilasiz.\n\n"
    "✦ Hisobingizni faollashtirish uchun birinchi depozitni amalga oshiring. "
    "Ushbu mablag‘lar HISOBINGIZGA kiritiladi, shundan so‘ng ularni o‘yin "
    "uchun va eng muhimi, g‘alaba qozonish uchun ishlatishingiz mumkin.\n\n"
    "● Birinchi depozitni amalga oshirganingizdan so‘ng, "
    "«🔍 Depozitni tekshirish» tugmasini bosing."
)


def welcome_text(lang: str, name: str) -> str:
    return t(lang, "welcome", name=name or "Do‘st")


async def send_photo_screen(target, photo: str, text: str, reply_markup=None):
    """Rasm + matn yuborish. target = Message yoki CallbackQuery."""
    try:
...
async def on_set_lang(cq: CallbackQuery):
    try:
        ...
        await cq.message.answer(t("uz", "choose_lang"), reply_markup=kb_langs())
    await cq.answer()
...
async def on_guide(cq: CallbackQuery):
    ...
    await send_photo_screen(cq, PHOTO_GUIDE, t(lang, "guide"), kb_guide(lang))
    await cq.answer()
...
async def on_account(cq: CallbackQuery, bot: Bot):
    try:
        ...
        try:
            await cq.message.edit_text(text, reply_markup=kb_account(lang))
        except Exception:
            await cq.message.answer(text, reply_markup=kb_account(lang))
    except Exception as e:
        logger.exception("account error: %s", e)
    finally:
        try:
            await cq.answer()
        except Exception:
...
async def on_levels(cq: CallbackQuery):
    ...
    lang = user.get("lang") or "uz"
    try:
        await cq.message.edit_text(LEVELS_TEXT, reply_markup=kb_levels(lang))
    except Exception:
        await cq.message.answer(LEVELS_TEXT, reply_markup=kb_levels(lang))
    await cq.answer()
...
async def on_signal(cq: CallbackQuery, bot: Bot):
    ...
    if not user["deposited"]:
        await send_photo_screen(cq, PHOTO_DEP, DEP_NEED_FULL, kb_deposit_check(lang, tg_id))
        await cq.answer()
        return

    if not user.get("balance_ok", 1):
        await send_photo_screen(
            cq, PHOTO_DEP, t(lang, "balance_zero"), kb_deposit(lang, tg_id)
        )
        await cq.answer()
        return
    ...
    try:
        ...
        await cq.message.answer(t(lang, "dep_ok"), reply_markup=kb_open_app(lang))
    await cq.answer()
...
async def on_check_id(cq: CallbackQuery):
    ...
    try:
        ...
        await cq.message.answer(t(user["lang"], "send_id"), reply_markup=kb_back(user["lang"]))
    await cq.answer()
...
async def on_dep_how(cq: CallbackQuery):
    ...
    lang = user.get("lang") or "uz"
    await send_photo_screen(
        cq, PHOTO_DEP, DEP_NEED_FULL, kb_deposit_check(lang, cq.from_user.id)
    )
    await cq.answer()
...
async def on_check_dep(cq: CallbackQuery):
    ...
    if user.get("deposited"):
        ...
        await send_photo_screen(
            cq, PHOTO_DEP, DEP_NEED_FULL, kb_deposit_check(lang, tg_id)
        )
        await cq.answer()


@router.message(F.photo)
async def on_photo_file_id(message: Message):
