...
# Telegram photo file_id (admin yuborgan rasmlar)
...
async def on_lang(cq: CallbackQuery, bot: Bot):
    ...
    if sub:
        ...
        await send_photo_screen(cq, PHOTO_START, t(lang, "welcome"), kb_main(lang))
    else:
        try:
...
async def on_check_dep(cq: CallbackQuery):
    ...
    if user.get("deposited"):
        ...
        # Popup emas — rasm + matn (TOWER botdagidek)
        await send_photo_screen(cq, PHOTO_DEP, DEP_NEED_FULL, kb_deposit_check(lang, tg_id))
        await cq.answer()
...
async def on_photo_file_id(message: Message):
    """Admin rasm yuborsa file_id qaytaradi."""
    if message.from_user.id not in ADMIN_IDS:
        return
    file_id = message.photo[-1].file_id
    await message.answer(
        f"✅ <b>file_id:</b>\n<code>{file_id}</code>\n\n"
        f"Shu kodni nusxalab saqlang."
    )
...
async def on_text(message: Message):
    ...
    if user.get("waiting_id"):
        ...
        return
    await send_photo_screen(
        message, PHOTO_START, t(user["lang"], "welcome"), kb_main(user["lang"])
    )
