...
async def access_handler(request: web.Request) -> web.Response:
    ...
    # CORS for GitHub Pages mini-app
    headers = {
    ...
    tg_raw = request.rel_url.query.get("tg_id") or ""
    if not tg_raw.isdigit():
        return web.json_response({"ok": False, "reason": "no_id"}, headers=headers)

    tg_id = int(tg_raw)
    user = await get_user(tg_id)
    ok = bool(user.get("registered") and user.get("deposited") and user.get("balance_ok", 1))
    reason = "ok"
    ...
    return web.json_response(
        {
            "ok": ok,
            "reason": reason,
            "registered": bool(user.get("registered")),
            "deposited": bool(user.get("deposited")),
            "balance_ok": bool(user.get("balance_ok", 1)),
        },
        headers=headers,
    )
