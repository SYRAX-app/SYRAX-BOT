...
T = {
    ...
    "es": {
        ...
        "welcome": "👋 ¡Bienvenido a <b>Syrax Signals</b>, {name}!",
        "btn_guide": "📖 Guía",
    ...
    "az": {
        ...
        "welcome": "👋 <b>Syrax Signals</b>-ə xoş gəlmisiniz, {name}!",
        "btn_guide": "📖 Təlimat",
    ...
    "pt_br": {
        ...
        "welcome": "👋 Bem-vindo ao <b>Syrax Signals</b>, {name}!",
        "btn_guide": "📖 Guia",
    ...
    "th": {
        ...
        "welcome": "👋 ยินดีต้อนรับสู่ <b>Syrax Signals</b>, {name}!",
        "btn_guide": "📖 คู่มือ",
    ...
    "ko": {
        ...
        "welcome": "👋 <b>Syrax Signals</b>에 오신 것을 환영합니다, {name}!",
        "btn_guide": "📖 가이드",
        "btn_signal": "❗ 시그널 받기 ❗",
        "btn_account": "🆔 내 계정",
        "btn_lang": "🌐 언어",
...
DEP_NEED_FULL = (
    ...
    "✦ Hisobingizni faollashtirish uchun birinchi depozitni amalga oshiring. "
    "Ushbu mablag‘lar HISOBINGIZGA kiritiladi.\n\n"
    "● Depozitdan so‘ng «🔍 Depozitni tekshirish» tugmasini bosing."
)
...
def welcome_text(lang: str, name: str) -> str:
    try:
        ...
        return t(lang, "welcome", name="User")


async def safe_screen(cq: CallbackQuery, text: str, reply_markup=None, photo: str | None = None):
    """Rasm yoki matn ekran — hech qachon 'ochilmaydi' holatiga tushmasin."""
    text = (text or "")[:1024] if photo else (text or "")
    ...
    try:
        if photo:
            ...
            return
        # matn ekran: agar oldingi xabar rasm bo‘lsa — o‘chirib yangi yuboramiz
        if cq.message.photo:
            try:
                await cq.message.delete()
            except Exception:
        ...
        logger.warning("safe_screen error: %s", e)
        try:
            await bot.send_message(chat_id, text, reply_markup=reply_markup)
        except Exception:
            pass
...
async def on_link_fail(cq: CallbackQuery):
    ...
    await safe_screen(
        cq,
        t(lang, "link_fail_help", support=SUPPORT_URL),
        kb_reg(lang, cq.from_user.id),
    )
    await cq.answer()
